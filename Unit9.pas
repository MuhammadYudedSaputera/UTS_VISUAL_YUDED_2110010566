...
procedure TFormTambahdata.btn5Click(Sender: TObject);
begin
Close;
end;
...
begin
  ...
  edt3.Text:=ds2.FieldValues['jam_mulai']; // Mengakses kolom penyewa
  edt4.Text:=ds2.FieldValues['jam_akhir']; // Mengakses kolom kode mobil
  cbb1.Text:=ds2.FieldValues['hari']; // Mengaskes kolom nama mobil
  dtp1.Date:=ds2.FieldValues['tanggal']; // mengakses kolom harga per hari
  edt5.Text:=ds2.FieldValues['ruang'];
  edt6.Text:=ds2.FieldValues['kehadiran_total']; // mengakses kolom no plat

  btn1.Enabled := False; // Mengakatifkan btn1
  btn2.Enabled := False; // Mengakatifkan btn2
  btn3.Enabled := True;// Menonaktifkan btn3
  btn4.Enabled := True; // Mengakatifkan btn4
  btn5.Enabled := True; // Mengakatifkan btn5
end;

procedure TFormTambahdata.FormCreate(Sender: TObject);
begin
edt1.Clear; // Mengosongkan edt1
edt2.Clear; // Mengosongkan edt2
edt3.Clear; // Mengosongkan edt3
